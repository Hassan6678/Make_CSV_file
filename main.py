...
def writeCSV(area,ratio,solidity,m1,m2,m3,m4,m5,m6,m7,target):
    if os.path.exists('Natural_Image_features.csv'):
        with open('Natural_Image_features.csv', mode='a', newline='') as csv_file:
...
if os.path.exists('Natural_Image_features.csv'):
    os.remove('Natural_Image_features.csv')  # if old file exist first delete that file then create New File
    with open('Natural_Image_features.csv', mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
